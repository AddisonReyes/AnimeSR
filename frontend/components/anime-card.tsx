    ...
    <button className="anime-card" onClick={() => onClick(anime.anime_id)} type="button">
      <div className="anime-card__poster">
        {anime.image_url ? (
          ...
          <div className="anime-card__fallback">Sin imagen</div>
        )}
      </div>

      <div className="anime-card__content">
        <div className="anime-card__eyebrow">
          <span>{anime.type ?? "Anime"}</span>
          <span>{anime.episodes ? `${anime.episodes} eps` : "Episodios N/D"}</span>
        </div>
        ...
        <div className="anime-card__stats">
          <span className="anime-card__score">★ {anime.score?.toFixed(2) ?? "N/D"}</span>
        </div>
